...
public final class LongSaturatedMathKt {
    ...
    public static final long m8653saturatingAddNuflL3o(long value, DurationUnit unit, long duration) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        long durationInUnit = Duration.m8590toLongimpl(duration, unit);
        if (((value - 1) | 1) == Long.MAX_VALUE) {
            return m8652checkInfiniteSumDefinedPjuGub4(value, duration, durationInUnit);
        }
        if (((durationInUnit - 1) | 1) == Long.MAX_VALUE) {
            return m8654saturatingAddInHalvesNuflL3o(value, unit, duration);
        }
    ...
    private static final long m8652checkInfiniteSumDefinedPjuGub4(long value, long duration, long durationInUnit) {
        if (!Duration.m8576isInfiniteimpl(duration) || (value ^ durationInUnit) >= 0) {
            return value;
        }
    ...
    private static final long m8654saturatingAddInHalvesNuflL3o(long value, DurationUnit unit, long duration) {
        long half = Duration.m8554divUwyO8pc(duration, 2);
        long halfInUnit = Duration.m8590toLongimpl(half, unit);
        if ((1 | (halfInUnit - 1)) == Long.MAX_VALUE) {
            return halfInUnit;
        }
        return m8653saturatingAddNuflL3o(m8653saturatingAddNuflL3o(value, unit, half), unit, Duration.m8579minusLRDsOJo(duration, half));
    }

    private static final long infinityOfSign(long value) {
        return value < 0 ? Duration.INSTANCE.m8641getNEG_INFINITEUwyO8pc$kotlin_stdlib() : Duration.INSTANCE.m8640getINFINITEUwyO8pc();
    }

    public static final long saturatingDiff(long valueNs, long origin, DurationUnit unit) {
        Intrinsics.checkNotNullParameter(unit, "unit");
        if ((1 | (origin - 1)) == Long.MAX_VALUE) {
            return Duration.m8595unaryMinusUwyO8pc(infinityOfSign(origin));
        }
    ...
    public static final long saturatingOriginsDiff(long origin1, long origin2, DurationUnit unit) {
        ...
        if ($i$f$isSaturated != 0) {
            return origin1 == origin2 ? Duration.INSTANCE.m8642getZEROUwyO8pc() : Duration.m8595unaryMinusUwyO8pc(infinityOfSign(origin2));
        }
    ...
    private static final long saturatingFiniteDiff(long value1, long value2, DurationUnit unit) {
        ...
        if (((result ^ value1) & (~(result ^ value2))) < 0) {
            if (unit.compareTo(DurationUnit.MILLISECONDS) < 0) {
                ...
                return Duration.m8580plusLRDsOJo(DurationKt.toDuration(resultMs, DurationUnit.MILLISECONDS), DurationKt.toDuration(resultUnit, unit));
            }
            return Duration.m8595unaryMinusUwyO8pc(infinityOfSign(result));
        }
