...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo3768alignKFBX0sM(long size, long space, LayoutDirection layoutDirection) {
        long remaining = IntSizeKt.IntSize(IntSize.m6796getWidthimpl(space) - IntSize.m6796getWidthimpl(size), IntSize.m6795getHeightimpl(space) - IntSize.m6795getHeightimpl(size));
        float centerX = IntSize.m6796getWidthimpl(remaining) / 2.0f;
        float centerY = IntSize.m6795getHeightimpl(remaining) / 2.0f;
        float x = (this.horizontalBias + 1.0f) * centerX;
        float y = (1.0f + this.verticalBias) * centerY;
        return IntOffsetKt.IntOffset(Math.round(x), Math.round(y));
    }
