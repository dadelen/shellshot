...
public final class IntListExtensionKt {
    ...
    public static final int binarySearch(IntList $this$binarySearch, int element, int fromIndex, int toIndex) {
        boolean value$iv = fromIndex <= toIndex;
        if (!value$iv) {
            PreconditionsKt.throwIllegalArgumentException("fromIndex(" + fromIndex + ") > toIndex(" + toIndex + ')');
        }
        if (fromIndex < 0) {
            throw new IndexOutOfBoundsException("Index out of range: " + fromIndex);
        }
        if (toIndex > $this$binarySearch.getSize()) {
            throw new IndexOutOfBoundsException("Index out of range: " + toIndex);
        }
        int low = fromIndex;
        int high = toIndex - 1;
        while (low <= high) {
            int mid = (low + high) >>> 1;
            int midVal = $this$binarySearch.get(mid);
            if (midVal < element) {
                low = mid + 1;
