...
public abstract class LazyGridMeasuredItemProvider implements LazyLayoutMeasuredItemProvider<LazyGridMeasuredItem> {
    ...
    public abstract LazyGridMeasuredItem mo811createItemO3s9Psw(int index, Object key, Object contentType, int crossAxisSize, int mainAxisSpacing, List<? extends Placeable> placeables, long constraints, int lane, int span);
    ...
    public LazyGridMeasuredItem mo786getAndMeasurehBUhpc(int index, int lane, int span, long constraints) {
        return m816getAndMeasurem8Kt_7k(index, constraints, lane, span, this.defaultMainAxisSpacing);
    }

    /* renamed from: getAndMeasure-m8Kt_7k, reason: not valid java name */
    public final LazyGridMeasuredItem m816getAndMeasurem8Kt_7k(int index, long constraints, int lane, int span, int mainAxisSpacing) {
        int crossAxisSize;
        ...
        if (Constraints.m6577getHasFixedWidthimpl(constraints)) {
            crossAxisSize = Constraints.m6581getMinWidthimpl(constraints);
        } else {
            if (!Constraints.m6576getHasFixedHeightimpl(constraints)) {
                throw new IllegalArgumentException("does not have fixed height".toString());
            }
            crossAxisSize = Constraints.m6580getMinHeightimpl(constraints);
        }
        return mo811createItemO3s9Psw(index, key, contentType, crossAxisSize, mainAxisSpacing, placeables, constraints, lane, span);
    }
