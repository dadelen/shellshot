...
public final class BringIntoViewRequestPriorityQueue {
    ...
    public final boolean enqueue(final ContentInViewNode.Request request) {
        ...
        if (requestBounds == null) {
            ...
            continuation.resumeWith(Result.m7212constructorimpl(Unit.INSTANCE));
            return false;
        }
        ...
        if (first <= i) {
            while (true) {
                ...
                ContentInViewNode.Request r = this_$iv2.getContent()[i];
    ...
    public final void resumeAndRemoveWhile(Function1<? super Rect, Boolean> block) {
        while (this.requests.isNotEmpty() && block.invoke(((ContentInViewNode.Request) this.requests.last()).getCurrentBounds().invoke()).booleanValue()) {
            ...
            MutableVector this_$iv = this.requests;
            CancellableContinuation<Unit> continuation = ((ContentInViewNode.Request) mutableVector.removeAt(this_$iv.getSize() - 1)).getContinuation();
            Unit unit = Unit.INSTANCE;
            Result.Companion companion = Result.INSTANCE;
            continuation.resumeWith(Result.m7212constructorimpl(unit));
        }
    ...
    public final void cancelAndRemoveAll(Throwable cause) {
        ...
        for (int i = 0; i < size; i++) {
            ContentInViewNode.Request it = this_$iv.getContent()[i];
