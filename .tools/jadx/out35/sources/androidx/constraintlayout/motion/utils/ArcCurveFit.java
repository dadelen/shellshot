...
class ArcCurveFit extends CurveFit {
    ...
    public void getPos(double t, float[] v) {
        ...
        for (int i = 0; i < this.mArcs.length; i++) {
            if (t <= this.mArcs[i].mTime2) {
                ...
                if (z) {
                    v[0] = (float) arcArr2[i].getLinearX(t);
        ...
        for (int i = 0; i < this.mArcs.length; i++) {
            if (t <= this.mArcs[i].mTime2) {
                if (this.mArcs[i].linear) {
                    Arc[] arcArr2 = this.mArcs;
                    if (j == 0) {
                        return arcArr2[i].getLinearX(t);
                    }
                    return arcArr2[i].getLinearY(t);
                }
                this.mArcs[i].setPoint(t);
                Arc[] arcArr3 = this.mArcs;
                if (j == 0) {
                    return arcArr3[i].getX();
                }
                return arcArr3[i].getY();
            }
        }
        return Double.NaN;
    }
    ...
    private static class Arc {
        ...
        Arc(int mode, double t1, double t2, double x1, double y1, double x2, double y2) {
            ...
            double dx = x2 - x1;
            double dy3 = y2 - y1;
            if (this.linear || Math.abs(dx) < EPSILON) {
                d = y1;
                d2 = x2;
                dy = dy3;
                dy2 = x1;
            } else {
                if (Math.abs(dy3) >= EPSILON) {
                    this.mLut = new double[101];
                    this.mEllipseA = (this.mVertical ? -1 : 1) * dx;
                    this.mEllipseB = (this.mVertical ? 1 : -1) * dy3;
