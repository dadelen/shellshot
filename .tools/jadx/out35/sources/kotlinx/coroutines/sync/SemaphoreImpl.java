    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        while (true) {
            ...
            Segment to$iv$iv = SegmentOrClosed.m8763getSegmentimpl(s$iv);
            while (true) {
                ...
                if (handler$atomicfu$iv.compareAndSet(this, cur$iv$iv, to$iv$iv)) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m8763getSegmentimpl(s$iv);
        i2 = SemaphoreKt.SEGMENT_SIZE;
        int i3 = (int) (enqIdx % i2);
        if (!segment.getAcquirers().compareAndSet(i3, null, waiter)) {
            symbol = SemaphoreKt.PERMIT;
            symbol2 = SemaphoreKt.TAKEN;
            if (segment.getAcquirers().compareAndSet(i3, symbol, symbol2)) {
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (!SegmentOrClosed.m8765isClosedimpl(findSegmentInternal)) {
                Segment m8763getSegmentimpl = SegmentOrClosed.m8763getSegmentimpl(findSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(this);
                    z = false;
                    z2 = true;
                    if (segment.id >= m8763getSegmentimpl.id) {
                        obj = 1;
                        break;
                    }
                    if (!m8763getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        obj = null;
                        break;
                    }
                    if (atomicReferenceFieldUpdater.compareAndSet(this, segment, m8763getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                        obj = 1;
                    } else if (m8763getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m8763getSegmentimpl.remove();
                    }
                }
                if (obj != null) {
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m8763getSegmentimpl(findSegmentInternal);
