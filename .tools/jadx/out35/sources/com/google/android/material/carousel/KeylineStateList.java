        ...
        if (isFirstFocalItemAtLeftOfContainer(defaultState)) {
            ...
            if (firstNonAnchorKeylineIndex != -1) {
                ...
                if (numberOfSteps <= 0 && defaultState.getFirstFocalKeyline().cutoff > 0.0f) {
                    float cutoffs2 = defaultState.getFirstFocalKeyline().cutoff;
                    steps.add(shiftKeylinesAndCreateKeylineState(defaultState, originalStart + cutoffs2, carouselSize));
                    return steps;
                }
                float cutoffs3 = 0.0f;
                int dstIndex = 0;
                while (dstIndex < numberOfSteps) {
                    KeylineState prevStepState = steps.get(steps.size() - i2);
                    int itemOrigIndex = firstNonAnchorKeylineIndex + dstIndex;
                    int dstIndex2 = defaultState.getKeylines().size() - i2;
                    float f2 = cutoffs;
                    float cutoffs4 = defaultState.getKeylines().get(itemOrigIndex).cutoff + cutoffs3;
                    if (itemOrigIndex - 1 < 0) {
                        i = i2;
                    } else {
                        i = i2;
                        float originalAdjacentMaskLeft = defaultState.getKeylines().get(itemOrigIndex - 1).mask;
                        dstIndex2 = findFirstIndexAfterLastFocalKeylineWithMask(prevStepState, originalAdjacentMaskLeft) - 1;
                    }
                    int newFirstFocalIndex = (defaultState.getFirstFocalKeylineIndex() - dstIndex) - 1;
                    int newLastFocalIndex = (defaultState.getLastFocalKeylineIndex() - dstIndex) - 1;
                    int newLastFocalIndex2 = dstIndex;
                    int i3 = dstIndex2;
                    KeylineState shifted = moveKeylineAndCreateKeylineState(prevStepState, firstNonAnchorKeylineIndex, i3, originalStart + cutoffs4, newFirstFocalIndex, newLastFocalIndex, carouselSize);
                    if (newLastFocalIndex2 == numberOfSteps - 1 && leftOrTopPaddingForKeylineShift > f2) {
                        shifted = shiftKeylineStateForPadding(shifted, leftOrTopPaddingForKeylineShift, carouselSize, i, itemMargins);
                    }
                    steps.add(shifted);
                    dstIndex = newLastFocalIndex2 + 1;
                    cutoffs3 = cutoffs4;
                    cutoffs = f2;
                    i2 = 1;
                }
        ...
        if (isLastFocalItemVisibleAtRightOfContainer(carousel, defaultState)) {
            ...
            if (lastNonAnchorKeylineIndex != -1) {
                ...
                while (dstIndex < numberOfSteps) {
                    ...
                    int itemOrigIndex = lastNonAnchorKeylineIndex - dstIndex;
                    ...
                    if (itemOrigIndex + 1 < defaultState.getKeylines().size()) {
                        ...
                        dstIndex2 = findLastIndexBeforeFirstFocalKeylineWithMask(prevStepState, originalAdjacentMaskRight) + 1;
                    }
                    int newFirstFocalIndex = defaultState.getFirstFocalKeylineIndex() + dstIndex + 1;
                    int newLastFocalIndex = defaultState.getLastFocalKeylineIndex() + dstIndex + 1;
                    int i3 = dstIndex;
                    int i4 = dstIndex2;
                    KeylineState shifted = moveKeylineAndCreateKeylineState(prevStepState, lastNonAnchorKeylineIndex, i4, originalStart - cutoffs3, newFirstFocalIndex, newLastFocalIndex, carouselSize);
                    if (i3 == numberOfSteps - 1 && rightOrBottomPaddingForKeylineShift > i2) {
