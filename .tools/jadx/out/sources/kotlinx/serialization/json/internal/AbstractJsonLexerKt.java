...
public final class AbstractJsonLexerKt {
    ...
    public static final String tokenDescription(byte token) {
        return token == 1 ? "quotation mark '\"'" : token == 2 ? "string escape sequence '\\'" : token == 4 ? "comma ','" : token == 5 ? "colon ':'" : token == 6 ? "start of the object '{'" : token == 7 ? "end of the object '}'" : token == 8 ? "start of the array '['" : token == 9 ? "end of the array ']'" : token == 10 ? "end of the input" : token == Byte.MAX_VALUE ? "invalid token" : "valid token";
    }
