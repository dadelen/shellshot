    ...
    */
    ...
    public final boolean addAcquireToQueue(Waiter waiter) {
        ...
        while (true) {
            ...
            Segment to$iv$iv = SegmentOrClosed.m8767getSegmentimpl(s$iv);
            while (true) {
        ...
        SemaphoreSegment segment = (SemaphoreSegment) SegmentOrClosed.m8767getSegmentimpl(s$iv);
    ...
    private final boolean tryResumeNextFromQueue() {
        ...
        while (true) {
            ...
            if (!SegmentOrClosed.m8769isClosedimpl(findSegmentInternal)) {
                Segment m8767getSegmentimpl = SegmentOrClosed.m8767getSegmentimpl(findSegmentInternal);
                while (true) {
                    Segment segment = (Segment) atomicReferenceFieldUpdater.get(this);
                    z = false;
                    z2 = true;
                    if (segment.id >= m8767getSegmentimpl.id) {
                        obj = 1;
                        break;
                    }
                    if (!m8767getSegmentimpl.tryIncPointers$kotlinx_coroutines_core()) {
                        obj = null;
                        break;
                    }
                    if (atomicReferenceFieldUpdater.compareAndSet(this, segment, m8767getSegmentimpl)) {
                        if (segment.decPointers$kotlinx_coroutines_core()) {
                            segment.remove();
                        }
                        obj = 1;
                    } else if (m8767getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
                        m8767getSegmentimpl.remove();
                    }
                }
                if (obj != null) {
                    break;
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m8767getSegmentimpl(findSegmentInternal);
