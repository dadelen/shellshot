...
final class IntrinsicWidthNode extends IntrinsicSizeModifier {
    ...
    public long mo626calculateContentConstraintsl58MMJ0(MeasureScope $this$calculateContentConstraints_u2dl58MMJ0, Measurable measurable, long constraints) {
        ...
        if (this.width == IntrinsicSize.Min) {
            measuredWidth = measurable.minIntrinsicWidth(Constraints.m6578getMaxHeightimpl(constraints));
        } else {
            measuredWidth = measurable.maxIntrinsicWidth(Constraints.m6578getMaxHeightimpl(constraints));
        }
        if (measuredWidth < 0) {
            measuredWidth = 0;
        }
        return Constraints.INSTANCE.m6591fixedWidthOenEA2s(measuredWidth);
    }
