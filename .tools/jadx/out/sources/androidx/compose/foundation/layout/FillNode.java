...
final class FillNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo84measure3p2s80s(MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        ...
        int minHeight;
        int height;
        if (Constraints.m6575getHasBoundedWidthimpl(constraints) && this.direction != Direction.Vertical) {
            float $this$fastRoundToInt$iv = Constraints.m6579getMaxWidthimpl(constraints) * this.fraction;
            int round = Math.round($this$fastRoundToInt$iv);
            int $i$f$fastRoundToInt = Constraints.m6581getMinWidthimpl(constraints);
            width = RangesKt.coerceIn(round, $i$f$fastRoundToInt, Constraints.m6579getMaxWidthimpl(constraints));
            minWidth = width;
        } else {
            minWidth = Constraints.m6581getMinWidthimpl(constraints);
            width = Constraints.m6579getMaxWidthimpl(constraints);
        }
        if (Constraints.m6574getHasBoundedHeightimpl(constraints) && this.direction != Direction.Horizontal) {
            float $this$fastRoundToInt$iv2 = Constraints.m6578getMaxHeightimpl(constraints) * this.fraction;
            int round2 = Math.round($this$fastRoundToInt$iv2);
            int $i$f$fastRoundToInt2 = Constraints.m6580getMinHeightimpl(constraints);
            height = RangesKt.coerceIn(round2, $i$f$fastRoundToInt2, Constraints.m6578getMaxHeightimpl(constraints));
            minHeight = height;
        } else {
            minHeight = Constraints.m6580getMinHeightimpl(constraints);
            height = Constraints.m6578getMaxHeightimpl(constraints);
        }
        final Placeable placeable = measurable.mo5491measureBRTryo0(ConstraintsKt.Constraints(minWidth, width, minHeight, height));
