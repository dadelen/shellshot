...
public final class FlowRowScopeInstance implements RowScope, FlowRowScope {
    ...
    public Modifier fillMaxRowHeight(Modifier $this$fillMaxRowHeight, float fraction) {
        if (!(((double) fraction) >= 0.0d)) {
            throw new IllegalArgumentException(("invalid fraction " + fraction + "; must be greater than or equal to zero").toString());
        }
        if (!(((double) fraction) <= 1.0d)) {
            throw new IllegalArgumentException(("invalid fraction " + fraction + "; must not be greater than 1.0").toString());
        }
