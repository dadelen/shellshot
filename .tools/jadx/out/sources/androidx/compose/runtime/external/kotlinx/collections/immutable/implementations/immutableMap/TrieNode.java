...
public final class TrieNode<K, V> {
    ...
    private final TrieNode<K, V> mutableInsertEntryAt(int positionMask, K key, V value, MutabilityOwnership owner) {
        ...
        if (mutabilityOwnership == owner) {
            insertEntryAtIndex = TrieNodeKt.insertEntryAtIndex(objArr, keyIndex, key, value);
            this.buffer = insertEntryAtIndex;
            this.dataMap |= positionMask;
            return this;
        }
        newBuffer = TrieNodeKt.insertEntryAtIndex(objArr, keyIndex, key, value);
    ...
    private final TrieNode<K, V> updateNodeAtIndex(int nodeIndex, int positionMask, TrieNode<K, V> newNode) {
        ...
        Object[] newBuffer2 = Arrays.copyOf(this.buffer, this.buffer.length);
    ...
    private final TrieNode<K, V> removeNodeAtIndex(int nodeIndex, int positionMask) {
        ...
        newBuffer = TrieNodeKt.removeNodeAtIndex(this.buffer, nodeIndex);
    ...
    private final TrieNode<K, V> mutableCollisionPutAll(TrieNode<K, V> otherNode, DeltaCounter intersectionCounter, MutabilityOwnership owner) {
        CommonFunctionsKt.m3748assert(this.nodeMap == 0);
        CommonFunctionsKt.m3748assert(this.dataMap == 0);
        CommonFunctionsKt.m3748assert(otherNode.nodeMap == 0);
        CommonFunctionsKt.m3748assert(otherNode.dataMap == 0);
