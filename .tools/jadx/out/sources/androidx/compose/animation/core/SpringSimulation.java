...
public final class SpringSimulation {
    ...
    private final void init() {
        ...
        if (f > 1.0f) {
            this.gammaPlus = ((-f2) * this.naturalFreq) + (this.naturalFreq * Math.sqrt(dampingRatioSquared - 1.0d));
            this.gammaMinus = ((-this.dampingRatio) * this.naturalFreq) - (this.naturalFreq * Math.sqrt(dampingRatioSquared - 1.0d));
        } else if (f2 >= 0.0f && this.dampingRatio < 1.0f) {
            this.dampedFreq = this.naturalFreq * Math.sqrt(1.0d - dampingRatioSquared);
        }
        this.initialized = true;
    }

    /* renamed from: updateValues-IJZedt4$animation_core_release, reason: not valid java name */
    public final long m161updateValuesIJZedt4$animation_core_release(float lastDisplacement, float lastVelocity, long timeElapsed) {
        double displacement;
        ...
        if (this.dampingRatio > 1.0f) {
            ...
            if (this.dampingRatio == 1.0f) {
                ...
                displacement = Math.exp((-this.naturalFreq) * deltaT) * (adjustedDisplacement + (coeffB2 * deltaT));
                ...
                double sinCoeff = (1.0d / this.dampedFreq) * ((this.dampingRatio * this.naturalFreq * adjustedDisplacement) + lastVelocity);
                displacement = Math.exp((-this.dampingRatio) * this.naturalFreq * deltaT) * ((adjustedDisplacement * Math.cos(this.dampedFreq * deltaT)) + (Math.sin(this.dampedFreq * deltaT) * sinCoeff));
                currentVelocity = ((-this.naturalFreq) * displacement * this.dampingRatio) + (Math.exp((-this.dampingRatio) * this.naturalFreq * deltaT) * (((-this.dampedFreq) * adjustedDisplacement * Math.sin(this.dampedFreq * deltaT)) + (Math.cos(this.dampedFreq * deltaT) * this.dampedFreq * sinCoeff)));
            }
        }
