...
public final class SplineBasedFloatDecayAnimationSpec_androidKt {
    ...
    public static final <T> DecayAnimationSpec<T> rememberSplineBasedDecay(Composer $composer, int $changed) {
        ...
        if (invalid$iv || it$iv == Composer.INSTANCE.getEmpty()) {
            value$iv = DecayAnimationSpecKt.generateDecayAnimationSpec(new SplineBasedFloatDecayAnimationSpec(density));
            $composer.updateRememberedValue(value$iv);
        } else {
            value$iv = it$iv;
        }
        DecayAnimationSpec<T> decayAnimationSpec = (DecayAnimationSpec) value$iv;
        ComposerKt.sourceInformationMarkerEnd($composer);
        if (ComposerKt.isTraceInProgress()) {
