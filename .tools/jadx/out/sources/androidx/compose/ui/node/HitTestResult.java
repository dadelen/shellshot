...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long distance = m5634findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m5629getDistanceimpl(distance) < 0.0f && DistanceAndInLayer.m5631isInLayerimpl(distance);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        long bestDistance = m5634findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m5625compareToS_HNhKs(bestDistance, distanceAndInLayer) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m5634findBestHitDistanceptXAw2c() {
        long bestDistance;
        ...
        if (i <= lastIndex) {
            while (true) {
                long distance = DistanceAndInLayer.m5626constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                bestDistance = DistanceAndInLayer.m5625compareToS_HNhKs(distance, bestDistance) < 0 ? distance : bestDistance;
                if (DistanceAndInLayer.m5629getDistanceimpl(bestDistance) >= 0.0f || !DistanceAndInLayer.m5631isInLayerimpl(bestDistance)) {
                    if (i == lastIndex) {
                        break;
                    }
                    i++;
    ...
    public final void hit(Modifier.Node node, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        if ((coordinator == null || coordinator.shouldSharePointerInputWithSiblings()) ? false : true) {
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        long previousDistance = m5634findBestHitDistanceptXAw2c();
        int previousHitDepth = this.hitDepth;
        this.hitDepth = CollectionsKt.getLastIndex(this);
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(this) && DistanceAndInLayer.m5625compareToS_HNhKs(previousDistance, m5634findBestHitDistanceptXAw2c()) > 0) {
