    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> list) {
        ...
        if (list.size() == 0 || list.size() == 1) {
            ...
            for (int i$iv = 0; i$iv < lastIndex; i$iv++) {
                ...
                result$iv.add(Offset.m3911boximpl(OffsetKt.Offset(Math.abs(Offset.m3922getXimpl(el1.getBoundsInRoot().m3952getCenterF1C5BW0()) - Offset.m3922getXimpl(el2.getBoundsInRoot().m3952getCenterF1C5BW0())), Math.abs(Offset.m3923getYimpl(el1.getBoundsInRoot().m3952getCenterF1C5BW0()) - Offset.m3923getYimpl(el2.getBoundsInRoot().m3952getCenterF1C5BW0())))));
                current$iv = next$iv;
            }
        }
        if (result$iv.size() == 1) {
            packedValue = ((Offset) CollectionsKt.first(result$iv)).getPackedValue();
        } else {
            List $this$fastReduce$iv = result$iv;
            ...
            if (1 <= lastIndex2) {
                while (true) {
                    long element = ((Offset) $this$fastReduce$iv.get(i$iv2)).getPackedValue();
                    long result = ((Offset) accumulator$iv).getPackedValue();
                    accumulator$iv = Offset.m3911boximpl(Offset.m3927plusMKHz9U(result, element));
                    if (i$iv2 == lastIndex2) {
                        break;
                    }
                    i$iv2++;
        ...
        float deltaX = Offset.m3922getXimpl(packedValue);
        float deltaY = Offset.m3923getYimpl(packedValue);
