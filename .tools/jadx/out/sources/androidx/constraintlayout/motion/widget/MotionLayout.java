...
public class MotionLayout extends ConstraintLayout implements NestedScrollingParent3 {
        ...
        if (stagger != 0.0f) {
            ...
            if (useMotionStagger) {
                ...
                for (int i5 = 0; i5 < n; i5++) {
                    MotionController f2 = this.mFrameArrayList.get(getChildAt(i5));
                ...
                for (int i6 = 0; i6 < n; i6++) {
                    MotionController f3 = this.mFrameArrayList.get(getChildAt(i6));
            ...
            int i7 = 0;
            while (i7 < n) {
                MotionController f4 = this.mFrameArrayList.get(getChildAt(i7));
                float x2 = f4.getFinalX();
                float y2 = f4.getFinalY();
                float mdist2 = flip ? y2 - x2 : y2 + x2;
                int i8 = i4;
                f4.mStaggerScale = i8 / (i4 - stagger2);
    ...
    */
    public void transitionToState(int r20, int r21, int r22) {
        /*
    ...
    public void getViewVelocity(View view, float posOnViewX, float posOnViewY, float[] returnVelocity, int type) {
        ...
        if (this.mInterpolator == null) {
            ...
            v = (dir * ((interpos - position2) / EPSILON)) / this.mTransitionDuration;
            deltaT = position2;
        }
    ...
    class Model {
        ...
        public void measure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            int width2 = width;
            int width3 = height;
            boolean isWidthMeasuredTooSmall2 = isWidthMeasuredTooSmall;
            boolean isWidthMeasuredTooSmall3 = isHeightMeasuredTooSmall;
            MotionLayout.this.resolveMeasuredDimension(widthMeasureSpec, heightMeasureSpec, width2, width3, isWidthMeasuredTooSmall2, isWidthMeasuredTooSmall3);
        }
    ...
    private void evaluateLayout() {
        ...
        float position2 = this.mTransitionLastPosition + deltaPos;
        boolean done = false;
        if (this.mTransitionInstantly) {
            position2 = this.mTransitionGoalPosition;
        }
        if ((dir > 0.0f && position2 >= this.mTransitionGoalPosition) || (dir <= 0.0f && position2 <= this.mTransitionGoalPosition)) {
            position2 = this.mTransitionGoalPosition;
            done = true;
        }
        if (this.mInterpolator != null && !done) {
            if (this.mTemporalInterpolator) {
                ...
                position2 = this.mInterpolator.getInterpolation(time);
            } else {
                position2 = this.mInterpolator.getInterpolation(position2);
            }
        }
        if ((dir > 0.0f && position2 >= this.mTransitionGoalPosition) || (dir <= 0.0f && position2 <= this.mTransitionGoalPosition)) {
            float position3 = this.mTransitionGoalPosition;
            position = position3;
        } else {
            position = position2;
        }
    ...
    void evaluate(boolean force) {
        ...
        if (this.mKeepAnimating || (this.mInTransition && (force || this.mTransitionGoalPosition != this.mTransitionLastPosition))) {
            ...
            if (this.mInterpolator == null || done) {
                ...
                if (!(this.mInterpolator instanceof MotionInterpolator)) {
                    ...
                    float abs = Math.abs(lastVelocity);
                    f = EPSILON;
                    if (abs * this.mTransitionDuration <= EPSILON) {
                        this.mInTransition = false;
                    }
            ...
            if (Math.abs(this.mLastVelocity) > f) {
                setState(TransitionState.MOVING);
            }
            if ((dir > 0.0f && position >= this.mTransitionGoalPosition) || (dir <= 0.0f && position <= this.mTransitionGoalPosition)) {
                position = this.mTransitionGoalPosition;
            ...
            f2 = f4;
            boolean end = (dir > f2 && position >= this.mTransitionGoalPosition) || (dir <= f2 && position <= this.mTransitionGoalPosition);
            ...
            this.mKeepAnimating |= end ? false : true;
            if (position <= f2 && this.mBeginState != -1 && this.mCurrentState != this.mBeginState) {
                newState = true;
                this.mCurrentState = this.mBeginState;
            ...
            if (this.mKeepAnimating || this.mInTransition) {
                invalidate();
            } else if ((dir > f2 && position == 1.0f) || (dir < f2 && position == f2)) {
                setState(TransitionState.FINISHED);
            }
            if ((!this.mKeepAnimating && this.mInTransition && dir > f2 && position == 1.0f) || (dir < f2 && position == f2)) {
                onNewStateAttachHandlers();
            }
        } else {
            f2 = 0.0f;
        ...
        if (this.mTransitionLastPosition >= 1.0f) {
            ...
            this.mCurrentState = this.mEndState;
        } else if (this.mTransitionLastPosition <= f2) {
            if (this.mCurrentState != this.mBeginState) {
                newState = true;
            }
            this.mCurrentState = this.mBeginState;
