    ...
    private void drawLine(Canvas canvas, Paint paint, float startFraction, float endFraction, int paintColor, int startGapSize, int endGapSize) {
        ...
        if (startPx <= endPx) {
            ...
            if (startBlockCenterX >= endBlockCenterX) {
                drawRoundedBlock(canvas, paint, new PointF(startBlockCenterX + originX, 0.0f), new PointF(endBlockCenterX + originX, 0.0f), blockWidth2, this.displayedTrackThickness);
                return;
            }
            ...
            if (!this.useStrokeCap && this.displayedCornerRadius > 0.0f) {
                ...
                if (startBlockCenterX > 0.0f) {
                    ...
                    linearDrawingDelegate.drawRoundedBlock(canvas, paint, new PointF(startBlockCenterX + originX, 0.0f), blockWidth, this.displayedTrackThickness);
                } else {
                    linearDrawingDelegate = this;
                    blockWidth = blockWidth2;
                }
                if (endBlockCenterX < linearDrawingDelegate.trackLength) {
                    linearDrawingDelegate.drawRoundedBlock(canvas, paint, new PointF(endBlockCenterX + originX, 0.0f), blockWidth, linearDrawingDelegate.displayedTrackThickness);
                }
            }
        }
    }
