...
public final class ForegroundAppResolver {
    ...
    private final boolean isCategoryGame(String packageName) {
        ...
        try {
            ...
            m7216constructorimpl = Result.m7216constructorimpl(Boolean.valueOf(this.context.getPackageManager().getApplicationInfo(packageName, 0).category == 0));
        } catch (Throwable th) {
            Result.Companion companion2 = Result.INSTANCE;
            m7216constructorimpl = Result.m7216constructorimpl(ResultKt.createFailure(th));
        }
        if (Result.m7219exceptionOrNullimpl(m7216constructorimpl) != null) {
            this.logger.d(TAG, "前台应用分类判断失败 package=" + packageName);
        }
        if (Result.m7222isFailureimpl(m7216constructorimpl)) {
            m7216constructorimpl = false;
        }
        return ((Boolean) m7216constructorimpl).booleanValue();
    }
