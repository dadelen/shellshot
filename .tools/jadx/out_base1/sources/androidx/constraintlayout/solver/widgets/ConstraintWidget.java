...
public class ConstraintWidget {
    ...
    /* JADX WARN: Code restructure failed: missing block: B:126:0x0288, code lost:
        ...
        if (r45.mResolvedDimensionRatioSide != (-1)) goto L153;
     */
    ...
    /* JADX WARN: Removed duplicated region for block: B:216:0x0566  */
    ...
    */
    ...
    private void applyConstraints(LinearSystem system, boolean isHorizontal, boolean parentWrapContent, boolean oppositeParentWrapContent, boolean isTerminal, SolverVariable parentMin, SolverVariable parentMax, DimensionBehaviour dimensionBehaviour, boolean wrapContent, ConstraintAnchor beginAnchor, ConstraintAnchor endAnchor, int beginPosition, int dimension, int minDimension, int maxDimension, float bias, boolean useRatio, boolean inChain, boolean oppositeInChain, boolean inBarrier, int matchConstraintDefault, int oppositeMatchConstraintDefault, int matchMinDimension, int matchMaxDimension, float matchPercentDimension, boolean applyPosition) {
        ...
        int numConnections3 = isBeginConnected2 ? 0 + 1 : 0;
        if (isEndConnected2) {
            numConnections3++;
        }
        ...
        if (!variableSize) {
            if (wrapContent) {
                ...
                if (maxDimension < Integer.MAX_VALUE) {
                    linearSystem.addLowerThan(end, begin3, maxDimension, i4);
                }
            } else {
            ...
            if (matchConstraintDefault2 == 1) {
                ...
                endTarget2 = endTarget;
                ArrayRow createRowDimensionRatio = linearSystem.createRow().createRowDimensionRatio(end, begin3, percentEnd, end2, matchPercentDimension);
                end = end;
                linearSystem.addConstraint(createRowDimensionRatio);
                variableSize2 = false;
        ...
        if (!applyPosition) {
            ...
            if (!inChain) {
                if (!isBeginConnected && !isEndConnected && !isCenterConnected) {
                    ...
                    if (this.mVisibility == matchConstraintDefault5) {
                        return;
                    }
                    if (applyRangeCheck3) {
                    ...
                    if (parentWrapContent && variableSize2 && minDimension == 0 && matchMinDimension2 == 0) {
                        if (!variableSize2) {
                            ...
                            linearSystem.addGreaterThan(end, begin2, 0, matchConstraintDefault5);
                        } else {
                            i3 = 0;
                        }
                        linearSystem.addGreaterThan(end, begin2, i3, wrapStrength);
                    }
                } else {
                    endTarget3 = endTarget2;
                    wrapStrength = 5;
        ...
        if (numConnections2 < 2 && parentWrapContent && isTerminal2) {
            linearSystem.addGreaterThan(begin3, parentMin, 0, matchConstraintDefault3);
            ...
            if (applyEnd) {
                linearSystem.addGreaterThan(solverVariable, end, 0, matchConstraintDefault3);
            }
        }
    }
