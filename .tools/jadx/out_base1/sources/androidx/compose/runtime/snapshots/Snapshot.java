...
public abstract class Snapshot {
    ...
    public abstract void mo3750nestedActivated$runtime_release(Snapshot snapshot);

    /* renamed from: nestedDeactivated$runtime_release */
    public abstract void mo3751nestedDeactivated$runtime_release(Snapshot snapshot);

    public abstract void notifyObjectsInitialized$runtime_release();

    /* renamed from: recordModified$runtime_release */
    public abstract void mo3752recordModified$runtime_release(StateObject state);
    ...
    public final void unsafeLeave(Snapshot oldSnapshot) {
        SnapshotThreadLocal snapshotThreadLocal;
        snapshotThreadLocal = SnapshotKt.threadSnapshot;
        boolean value$iv = snapshotThreadLocal.get() == this;
        if (!value$iv) {
    ...
    public static final class Companion {
        ...
        public final <T> T observe(Function1<Object, Unit> readObserver, Function1<Object, Unit> writeObserver, Function0<? extends T> block) {
            ...
            if (readObserver != null || writeObserver != null) {
                ...
                if (previous instanceof TransparentObserverMutableSnapshot) {
                    ...
                    if ($this$canBeReused$iv.getThreadId() == ActualJvm_jvmKt.currentThreadId()) {
                        ...
                        try {
                            ((TransparentObserverMutableSnapshot) previous).setReadObserver(SnapshotKt.mergedReadObserver$default(readObserver, previousReadObserver, false, 4, null));
                            mergedWriteObserver = SnapshotKt.mergedWriteObserver(writeObserver, previousWriteObserver);
                            ((TransparentObserverMutableSnapshot) previous).setWriteObserver(mergedWriteObserver);
                            return block.invoke();
                        } finally {
        ...
        private final boolean getCanBeReused(TransparentObserverMutableSnapshot $this$canBeReused) {
            return $this$canBeReused.getThreadId() == ActualJvm_jvmKt.currentThreadId();
        }

        private final boolean getCanBeReused(TransparentObserverSnapshot $this$canBeReused) {
            return $this$canBeReused.getThreadId$runtime_release() == ActualJvm_jvmKt.currentThreadId();
        }

        public final Snapshot makeCurrentNonObservable(Snapshot previous) {
            if (previous instanceof TransparentObserverMutableSnapshot) {
                TransparentObserverMutableSnapshot $this$canBeReused$iv = (TransparentObserverMutableSnapshot) previous;
                TransparentObserverMutableSnapshot $this$canBeReused$iv2 = $this$canBeReused$iv.getThreadId() == ActualJvm_jvmKt.currentThreadId() ? 1 : null;
            ...
            if (previous instanceof TransparentObserverSnapshot) {
                ...
                if ($this$canBeReused$iv3.getThreadId$runtime_release() == ActualJvm_jvmKt.currentThreadId()) {
        ...
        static final void registerApplyObserver$lambda$6(Function2 $observer) {
            List list;
            Object lock$iv$iv = SnapshotKt.getLock();
            synchronized (lock$iv$iv) {
                list = SnapshotKt.applyObservers;
                SnapshotKt.applyObservers = CollectionsKt.minus(list, $observer);
                Unit unit = Unit.INSTANCE;
            }
        }
        ...
        static final void registerGlobalWriteObserver$lambda$9(Function1 $observer) {
            List list;
            Object lock$iv$iv = SnapshotKt.getLock();
            synchronized (lock$iv$iv) {
                list = SnapshotKt.globalWriteObservers;
                SnapshotKt.globalWriteObservers = CollectionsKt.minus(list, $observer);
                Unit unit = Unit.INSTANCE;
            }
            SnapshotKt.advanceGlobalSnapshot();
        }
