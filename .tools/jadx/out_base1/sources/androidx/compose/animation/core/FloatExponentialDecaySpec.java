...
public final class FloatExponentialDecaySpec implements FloatDecayAnimationSpec {
    ...
    */
    ...
    public float getTargetValue(float initialValue, float initialVelocity) {
        ...
        double duration = (Math.log(Math.abs(getAbsVelocityThreshold() / initialVelocity)) / this.friction) * 1000.0d;
        return (initialValue - (initialVelocity / this.friction)) + ((initialVelocity / this.friction) * ((float) Math.exp((this.friction * duration) / 1000.0d)));
    }
}
