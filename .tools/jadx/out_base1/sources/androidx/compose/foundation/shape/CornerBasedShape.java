...
public abstract class CornerBasedShape implements Shape {
    ...
    public abstract Outline mo938createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection);
    ...
    public final Outline mo275createOutlinePq9zytI(long size, LayoutDirection layoutDirection, Density density) {
        float topStart = this.topStart.mo946toPxTmRCtEA(size, density);
        float topEnd = this.topEnd.mo946toPxTmRCtEA(size, density);
        float bottomEnd = this.bottomEnd.mo946toPxTmRCtEA(size, density);
        float bottomStart = this.bottomStart.mo946toPxTmRCtEA(size, density);
        float minDimension = Size.m3990getMinDimensionimpl(size);
        if (topStart + bottomStart > minDimension) {
            float scale = minDimension / (topStart + bottomStart);
            topStart *= scale;
            bottomStart *= scale;
        }
        if (topEnd + bottomEnd > minDimension) {
            float scale2 = minDimension / (topEnd + bottomEnd);
            topEnd *= scale2;
            bottomEnd *= scale2;
        }
        if (topStart >= 0.0f && topEnd >= 0.0f && bottomEnd >= 0.0f && bottomStart >= 0.0f) {
            return mo938createOutlineLjSzlW0(size, topStart, topEnd, bottomEnd, bottomStart, layoutDirection);
        }
