...
final class WrapContentNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult mo84measure3p2s80s(final MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        int m6579getMaxWidthimpl;
        int m6581getMinWidthimpl = this.direction != Direction.Vertical ? 0 : Constraints.m6581getMinWidthimpl(constraints);
        int m6580getMinHeightimpl = this.direction == Direction.Horizontal ? Constraints.m6580getMinHeightimpl(constraints) : 0;
        if (this.direction != Direction.Vertical && this.unbounded) {
            m6579getMaxWidthimpl = Integer.MAX_VALUE;
        } else {
            m6579getMaxWidthimpl = Constraints.m6579getMaxWidthimpl(constraints);
        }
        long wrappedConstraints = ConstraintsKt.Constraints(m6581getMinWidthimpl, m6579getMaxWidthimpl, m6580getMinHeightimpl, (this.direction == Direction.Horizontal || !this.unbounded) ? Constraints.m6578getMaxHeightimpl(constraints) : Integer.MAX_VALUE);
        final Placeable placeable = measurable.mo5491measureBRTryo0(wrappedConstraints);
        final int wrapperWidth = RangesKt.coerceIn(placeable.getWidth(), Constraints.m6581getMinWidthimpl(constraints), Constraints.m6579getMaxWidthimpl(constraints));
        final int wrapperHeight = RangesKt.coerceIn(placeable.getHeight(), Constraints.m6580getMinHeightimpl(constraints), Constraints.m6578getMaxHeightimpl(constraints));
        return MeasureScope.layout$default($this$measure_u2d3p2s80s, wrapperWidth, wrapperHeight, null, new Function1<Placeable.PlacementScope, Unit>() { // from class: androidx.compose.foundation.layout.WrapContentNode$measure$1
            ...
            public final void invoke2(Placeable.PlacementScope $this$layout) {
                long position = WrapContentNode.this.getAlignmentCallback().invoke(IntSize.m6788boximpl(IntSizeKt.IntSize(wrapperWidth - placeable.getWidth(), wrapperHeight - placeable.getHeight())), $this$measure_u2d3p2s80s.getLayoutDirection()).getPackedValue();
