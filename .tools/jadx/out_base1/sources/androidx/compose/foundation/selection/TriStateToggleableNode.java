...
final class TriStateToggleableNode extends ClickableNode {
    ...
    private TriStateToggleableNode(ToggleableState state, MutableInteractionSource interactionSource, IndicationNodeFactory indicationNodeFactory, boolean enabled, Role role, Function0<Unit> function0) {
        super(interactionSource, indicationNodeFactory, enabled, null, role, function0, null);
        this.state = state;
    }

    /* renamed from: update-QzZPfjk, reason: not valid java name */
    public final void m937updateQzZPfjk(ToggleableState state, MutableInteractionSource interactionSource, IndicationNodeFactory indicationNodeFactory, boolean enabled, Role role, Function0<Unit> onClick) {
        if (this.state != state) {
            this.state = state;
            SemanticsModifierNodeKt.invalidateSemantics(this);
        }
        super.m268updateQzZPfjk(interactionSource, indicationNodeFactory, enabled, (String) null, role, onClick);
    }
