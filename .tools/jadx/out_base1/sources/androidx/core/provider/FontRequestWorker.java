...
class FontRequestWorker {
    ...
    static Typeface requestFontSync(final Context context, final FontRequest request, CallbackWrapper callback, final int style, int timeoutInMillis) {
        ...
        if (timeoutInMillis == -1) {
            TypefaceResult typefaceResult = getFontSync(id, context, List.of(request), style);
        ...
        Callable<TypefaceResult> fetcher = new Callable<TypefaceResult>() { // from class: androidx.core.provider.FontRequestWorker.1
            ...
            public TypefaceResult call() {
                return FontRequestWorker.getFontSync(id, context, List.of(request), style);
            }
        };
        try {
    ...
    static TypefaceResult getFontSync(String cacheId, Context context, List<FontRequest> requests, int style) {
        ...
        try {
            ...
            Typeface typeface = result.hasFallback() ? TypefaceCompat.createFromFontInfoWithFallback(context, null, result.getFontsWithFallbacks(), style) : TypefaceCompat.createFromFontInfo(context, null, result.getFonts(), style);
            if (typeface == null) {
                return new TypefaceResult(-3);
            }
